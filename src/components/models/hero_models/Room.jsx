...
export function Room(props) {
  ...
  // Optimized materials
  const curtainMaterial = useRef(new THREE.MeshPhongMaterial({ color: "#d90429" })).current;
  const bodyMaterial = useRef(new THREE.MeshPhongMaterial({ map: matcapTexture })).current;
  const tableMaterial = useRef(new THREE.MeshPhongMaterial({ color: "#582f0e" })).current;
  const radiatorMaterial = useRef(new THREE.MeshPhongMaterial({ color: "#fff" })).current;
  const compMaterial = useRef(new THREE.MeshStandardMaterial({ color: "#fff" })).current;
  const pillowMaterial = useRef(new THREE.MeshPhongMaterial({ color: "#8338ec" })).current;
  const chairMaterial = useRef(new THREE.MeshPhongMaterial({ color: "#000" })).current;

  return (
    <group {...props} dispose={null}>
      {/* Bloom effect only for screens */}
      <EffectComposer>
        <SelectiveBloom
          selection={screensRef}
          intensity={1} // reduced for performance
          luminanceThreshold={0.3} // slightly higher threshold
          luminanceSmoothing={0.7} // smoother transition
